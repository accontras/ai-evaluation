...
> **版本**: v1.0 | **日期**: 2026-06-05 | **作者**: accontras
...
评估系统采用**四层概念模型**，从底向上逐层抽象：

```
┌──────────────────────────────────────────────────────────────┐
│  场景（Scene）                                                │
│  将评估对象与评估模型关联，定义评估的执行上下文                  │
├──────────────────────────────────────────────────────────────┤
│  模型（Model）                                                │
│  评估模板：聚合多个指标 + 参考标准 + 事件规则                  │
│  一个模型对应一个业务评估场景                                  │
├──────────────────────────────────────────────────────────────┤
│  对象（Target）                                               │
│  模型的实例化：一个具体被评估的业务实体                        │
...
> 最初的设计争议点在于：一个模型是否应对应多个对象（类似"类与实例"的关系）？
>
> 最终选择一对一，理由：
> 1. 业务上每个评估场景的指标集、权重、参考标准都是独立配置的，强行复用模型会导致配置耦合
> 2. 一对一的模型定义最清晰，后续如需模板化，可在模型层之上再加"模型模板"层（类似于"类←→对象"的元层次）
> 3. 避免"修改一个模型影响多个评估场景"的连锁风险

### 2.2 参考标准机制（关键创新）

**问题**：指标 A 的标准值依赖于指标 B 的值。例如"日志填报率"的合格线取决于"入职天数"——新人（入职 < 30 天）的合格线是 60%，老人（入职 > 365 天）的合格线是 90%。

**传统方案**：在指标自身配置复杂的条件表达式。缺点是规则扩散到每个指标中，不可集中管理。

**本方案**：引入**参考标准（Reference Standard）** 层：

```
参考标准 = 一组"条件→标准值"映射规则
   └─ 独立于指标存在，可被多个指标引用
   └─ 条件基于其他指标的值动态匹配
   └─ 支持三级 fallback：模型专属 → 通用 → 默认 100 分
```
...
```
参考标准 "入职天数-日志填报率标准":
  IF 入职天数 < 30   → 合格线 = 60%, 满分线 = 90%
  IF 30 ≤ 入职天数 < 365 → 合格线 = 75%, 满分线 = 95%
  IF 入职天数 ≥ 365  → 合格线 = 90%, 满分线 = 100%
  ELSE → 默认合格线 = 80%
```

**设计价值**：将"动态规则"从指标配置中剥离，集中到参考标准层管理。指标只需要说"我的评分标准是 X"，X 的展开由参考标准引擎完成。当你需要调整某个维度的评价尺度时，改参考标准一处即可，无需遍历所有相关指标。
