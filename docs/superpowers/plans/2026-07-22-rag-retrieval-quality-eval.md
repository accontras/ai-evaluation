...
- [x] 1.1 从 eval_indicator_log 筛选 8-12 条代表性查询（覆盖不同指标类型和数值范围）
- [x] 1.2 合成 5-8 条边界查询（极端值、无数据、稀疏指标）
- [x] 1.3 标注每条查询的理想 Top-3 检索结果（人工判断历史案例相关性）
- [x] 1.4 创建 `data/rag-ground-truth.json`，含 id/indexCode/indexName/dataValue/relevantLogIds/notes

### Task 2: 数据库表 + Entity + Mapper

**Files:** Create DDL, Entity, Mapper

- [x] 2.1 创建 `docs/sql/V010__rag_compare_log.sql` DDL
- [x] 2.2 创建 Entity: `eval-domain/.../model/EvalRagCompareLog.java`（MyBatis-Plus 注解，手动 getter/setter）
- [x] 2.3 创建 Mapper: `eval-infrastructure/.../mapper/EvalRagCompareLogMapper.java`（继承 BaseMapper）
- [x] 2.4 编译验证: `mvn compile -q` (⏳ JDK 17 环境待安装)

### Task 3: RagCompareTracker 改造

**Files:** Modify RagCompareTracker.java, LlmScoringStrategy.java (编译适配)

- [x] 3.1 RagCompareTracker 新增 11 参数重载 `record()` 方法（含完整字段 + DB 写入）
- [x] 3.2 保留原 5 参数方法（@Deprecated），增加 DB 写入（ground_truth_rel=NULL）
- [x] 3.3 LlmScoringStrategy 编译适配（Spring 自动注入 handle 新依赖）
- [x] 3.4 编译验证: `mvn compile -q` (⏳ JDK 17 环境待安装)

### Task 4: 量化指标计算 RagQualityService

**Files:** Create RagQualityService.java, RagQualityServiceTest.java

- [x] 4.1 编写单元测试（TDD 先写测试，8 个用例：全命中/全不中/空输入/最佳排序/次优排序/全不相关/多查询/K>结果数）
- [x] 4.2 实现 RagQualityService.hrAtK() — Hit Rate@K 计算
- [x] 4.3 实现 RagQualityService.ndcgAtK() — NDCG@K 计算（二元相关，边界 DCG=0→NDCG=1.0）
- [x] 4.4 运行测试通过: `mvn test -pl eval-application -Dtest=RagQualityServiceTest` (⏳ JDK 17 环境待安装)

### Task 5: 跑批评测脚本 RagQualityEvalRunner

**Files:** Create RagQualityEvalRunner.java, modify application.yml

- [x] 5.1 创建 `RagQualityEvalRunner`（@Profile("rag-eval"), CommandLineRunner）
- [x] 5.2 application.yml 添加 rag-eval profile（web-application-type: none）
- [x] 5.3 实现 6 步工作流: 检查可用性 → 加载 ground truth → 逐条检索 → 汇总计算 → 输出 results.json → 打印控制台对比表
- [x] 5.4 编译验证: `mvn compile -q` (⏳ JDK 17 环境待安装)

### Task 6: 图表生成

**Files:** Create `data/rag-eval-results/index.html`

- [x] 6.1 创建 HTML 报告页面：汇总对比表 + Chart.js 柱状图（HR@K）+ 折线图（NDCG@K）
- [x] 6.2 通过 fetch('results.json') 加载评测数据动态渲染
