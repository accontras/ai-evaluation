...
**Scene = 方案，两个名字指同一件事**。系统核心只有三层：

```
┌──────────────────────────────────────────────────────────────┐
│  模型（Model）                                                │
│  评估模板：定义 Stage 树 + 指标池 + 事件规则                   │
│  Model 本身不直接用于评估——必须通过深拷贝创建 Scene/方案       │
├──────────────────────────────────────────────────────────────┤
│  方案 / 场景（Scene）  ← 同一概念，两个名字                     │
│  Scene = Model 的实例化副本 = 一个评估方案                      │
│  携带独立的 Stage 树副本，可独立修改而不影响 Model              │
│  通过 SceneCopyDomainService 深拷贝创建                        │
├──────────────────────────────────────────────────────────────┤
│  ★ Stage（评估阶段/维度树）  ← 系统核心                         │
│  树形结构，三种类型:                                           │
...
│                                                                │
│  整个评估计算就是 Stage 树的自底向上遍历。                      │
│  权重在 Stage 上。LLM 在 LEAF 停下。                           │
├──────────────────────────────────────────────────────────────┤
│  指标（Index）                                                │
│  叶子节点的数据点。指标自身不参与树结构——它只是 Stage 的数据输入│
└──────────────────────────────────────────────────────────────┘
```

**关键认知**：

> Scene 和 方案是同一个东西。DB 里叫 `eval_scene`，API 里叫"方案"，代码里叫 Scene——三个名字指的是同一个概念：从 Model 模板深拷贝出来的、可独立运行的评估配置副本。
>
> **Stage 才是系统的核心组织单元**。Model 提供 Stage 树的"模具"，Scene 持有 Stage 树的"副本"，而评估执行的每一步（H1 加载、H3 算分、H4 事件）都是在遍历和操作 Stage 树。

**设计决策：为什么模型与方案分离（深拷贝模式）？**

> Model 是模板，Scene/方案是实例。通过 `SceneCopyDomainService` 深拷贝 Model → Scene：
>
...
>
> 1. **树结构承载评估层次**：LEAF 收集指标分 → NORMAL 加权聚合 → TOP 路由分叉
> 2. **权重在 Stage 上，不在指标上**：同一个 Stage 下的指标等权，不同 Stage 按 weight 加权——评估的粒度是 Stage，不是指标
> 3. **LLM 和规则引擎的分界线**：LLM 在 LEAF 层打完分就停下，往上永不交给 LLM，这是审计底线
> 4. **TOP 路由是 Stage 级决策**：根据业务条件路由到不同子 Stage 分支，本质上是切换评估策略
