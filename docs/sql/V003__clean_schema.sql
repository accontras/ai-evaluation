-- ============================================
-- AI 评估系统 — 完整建库 DDL v3 (25 张表)
-- 变更:
--   1. 删 domain_code / catagory_code
--   2. 加 enabled (逻辑删除) / 保留 status (业务状态)
--   3. created_at → create_time, updated_at → update_time
-- ============================================
...
CREATE TABLE eval_model (
    ...
    status          VARCHAR(16)   DEFAULT 'DRAFT' COMMENT 'DRAFT / ENABLED / DISABLED',
    dimensions      VARCHAR(500)  COMMENT '维度定义 JSON',
    dimension_options VARCHAR(500),
    vn              INT           DEFAULT 1 COMMENT '乐观锁版本号',
    enabled         TINYINT       DEFAULT 1 COMMENT '逻辑删除: 1正常 0删除',
    memo            VARCHAR(255),
...
CREATE TABLE eval_model_stage (
    ...
    default_score   DECIMAL(10,2) COMMENT '叶子 stage 兜底得分',
    enabled         TINYINT       DEFAULT 1 COMMENT '逻辑删除',
    memo            VARCHAR(255),
    create_time     DATETIME      DEFAULT CURRENT_TIMESTAMP,
    update_time     DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE eval_model_event (
    ...
    target_id       BIGINT        COMMENT '挂载实体 ID',
...
CREATE TABLE eval_scene (
    ...
    status          VARCHAR(18)   DEFAULT 'DRAFT' COMMENT 'DRAFT / PUBLISHED / DISABLED',
...
CREATE TABLE eval_scene_stage (
    ...
    source_id       BIGINT        COMMENT '模板层原 ID',
...
CREATE TABLE eval_index (
    ...
    unit            VARCHAR(20),
    index_field_code VARCHAR(255) COMMENT 'API 返回字段名',
...
CREATE TABLE eval_decision_rule (
    ...
    type            VARCHAR(36),
    vn              INT           DEFAULT 1,
    status          VARCHAR(36)   COMMENT 'ENABLED / DISABLED',
    enabled         TINYINT       DEFAULT 1,
    memo            VARCHAR(255),
    create_time     DATETIME      DEFAULT CURRENT_TIMESTAMP,
    update_time     DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE eval_object_log (
    ...
    appeal_source   VARCHAR(64)   COMMENT '申诉重算源批次',
    status          VARCHAR(20),
    enabled         TINYINT       DEFAULT 1,
    memo            VARCHAR(255),
    create_time     DATETIME      DEFAULT CURRENT_TIMESTAMP,
    update_time     DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_task (task_log_id),
...
CREATE TABLE eval_appeal_header (
    ...
    task_log_id     BIGINT        COMMENT '关联 eval_task_log',
    enabled         TINYINT       DEFAULT 1,
    create_time     DATETIME      DEFAULT CURRENT_TIMESTAMP,
    update_time     DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE eval_appeal_detail (
    ...
    dimension_id    BIGINT        COMMENT '维度 ID (DIMENSION 用, MVP 留空)',
